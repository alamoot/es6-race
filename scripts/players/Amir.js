    ...
    move (track, me) {
       ...
       /* Avoiding obstacles by always turning left (and only right when we can't turn left anymore). */
        if (me.y >= 3 && track.now[me.y - 3][me.x] !== ' ') {
            ...
            if (track.now[me.y - 3][me.x] == '*'){
                if(( (track.now[me.y - 3][me.x - 1] == '*' && track.now[me.y - 3][me.x - 2] == '*') ||
                     (track.now[me.y - 2][me.x - 1])) && track.now[me.y - 2][me.x + 1] != '*')
                    decision.turn = 1;
                else if(( (track.now[me.y - 3][me.x + 1] == '*' && track.now[me.y - 3][me.x + 2] == '*') ||
                          (track.now[me.y - 2][me.x + 1] )) && track.now[me.y - 2][me.x - 1] == '*')
                    decision.turn = -1;
            }
            else if (track.now[me.y - 2][me.x] == '*'){
                if (track.now[me.y - 2][me.x - 1] == '*')
                    decision.turn = 1;
                else if (track.now[me.y - 2][me.x + 1] == '*')
                    decision.turn = -1;
            }
        }
